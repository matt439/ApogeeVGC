...
# Runs the program multiple times and compares outputs (filtering build warnings)
...
# First build the project once to avoid build output in runs
Write-Host "Building project..." -ForegroundColor Yellow
dotnet build .\ApogeeVGC\ApogeeVGC.csproj --no-restore -v quiet | Out-Null
...
for ($i = 1; $i -le $numRuns; $i++) {
    ...
    # Run the program (no build) and capture output
    dotnet run --project .\ApogeeVGC\ApogeeVGC.csproj --no-build 2>&1 | Out-File -FilePath $outputFile -Encoding UTF8
