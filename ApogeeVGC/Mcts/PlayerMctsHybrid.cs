...
/// <summary>
/// Hybrid MCTS player: uniform priors (like standalone) + DL value head for leaf evaluation.
/// Isolates the value head's contribution from the policy head's impact on search.
/// Uses team preview model if available.
/// </summary>
public sealed class PlayerMctsHybrid(
    SideId sideId,
    PlayerOptions options,
    IBattleController battleController,
    MctsSearchHybrid search)
    : IPlayer
{
    ...
    /// <summary>
    /// Factory method that creates a PlayerMctsHybrid using MctsResources.
    /// </summary>
    public static PlayerMctsHybrid Create(SideId sideId, PlayerOptions options, IBattleController battleController)
    {
        var search = new MctsSearchHybrid(
            options.MctsConfig ?? MctsResources.Config,
            MctsResources.Model);
        return new PlayerMctsHybrid(sideId, options, battleController, search);
    }
        ...
        Func<BattlePerspective> perspectiveFactory)
    {
        return choiceRequest switch
        {
            ...
            _ => throw new NotImplementedException(
                $"Request type {choiceRequest.GetType().Name} not implemented for hybrid MCTS"),
        };
    }
