...
public record Condition : ISideEventMethods, IFieldEventMethods, IPokemonEventMethods, IEffect, IBasicEffect
{
    ...
    public Delegate? GetDelegate(EventId id)
    {
        ...
        {
            EventId.End => OnEnd,
            EventId.Start => OnStart,
            EventId.Restart => OnRestart,
            EventId.DamagingHit => OnDamagingHit,
            EventId.EmergencyExit => OnEmergencyExit,
            EventId.AfterEachBoost => OnAfterEachBoost,
            EventId.AfterHit => OnAfterHit,
            EventId.AfterMega => OnAfterMega,
            EventId.AfterSetStatus => OnAfterSetStatus,
            EventId.AfterSubDamage => OnAfterSubDamage,
            EventId.AfterSwitchInSelf => OnAfterSwitchInSelf,
            EventId.AfterTerastallization => OnAfterTerastallization,
            EventId.AfterUseItem => OnAfterUseItem,
            EventId.AfterTakeItem => OnAfterTakeItem,
            EventId.AfterBoost => OnAfterBoost,
            EventId.AfterFaint => OnAfterFaint,
            ...
            EventId.AfterMoveSelf => OnAfterMoveSelf,
            EventId.Attract => OnAttract,
            EventId.Accuracy => OnAccuracy,
            EventId.BasePower => OnBasePower,
            EventId.BeforeFaint => OnBeforeFaint,
            EventId.BeforeMove => OnBeforeMove,
            EventId.BeforeSwitchIn => OnBeforeSwitchIn,
            EventId.BeforeSwitchOut => OnBeforeSwitchOut,
            EventId.BeforeTurn => OnBeforeTurn,
            EventId.ChangeBoost => OnChangeBoost,
            EventId.TryBoost => OnTryBoost,
            EventId.ChargeMove => OnChargeMove,
            EventId.CriticalHit => OnCriticalHit,
            EventId.Damage => OnDamage,
            EventId.DeductPp => OnDeductPp,
            EventId.DisableMove => OnDisableMove,
            EventId.DragOut => OnDragOut,
            EventId.EatItem => OnEatItem,
            EventId.Effectiveness => OnEffectiveness,
            EventId.EntryHazard => OnEntryHazard,
            EventId.Faint => OnFaint,
            EventId.Flinch => OnFlinch,
            EventId.FractionalPriority => OnFractionalPriority,
            EventId.Hit => OnHit,
            EventId.Immunity => OnImmunity,
            EventId.LockMove => OnLockMove,
            EventId.MaybeTrapPokemon => OnMaybeTrapPokemon,
            EventId.ModifyAccuracy => OnModifyAccuracy,
            EventId.ModifyAtk => OnModifyAtk,
            EventId.ModifyBoost => OnModifyBoost,
            EventId.ModifyCritRatio => OnModifyCritRatio,
            EventId.ModifyDamage => OnModifyDamage,
            EventId.ModifyDef => OnModifyDef,
            EventId.ModifyMove => OnModifyMove,
            EventId.ModifyPriority => OnModifyPriority,
            EventId.ModifySecondaries => OnModifySecondaries,
            EventId.ModifyType => OnModifyType,
            EventId.ModifyTarget => OnModifyTarget,
            EventId.ModifySpA => OnModifySpA,
            EventId.ModifySpD => OnModifySpD,
            EventId.ModifySpe => OnModifySpe,
            EventId.ModifyStab => OnModifyStab,
            EventId.ModifyWeight => OnModifyWeight,
            EventId.MoveAborted => OnMoveAborted,
            EventId.NegateImmunity => OnNegateImmunity,
            EventId.OverrideAction => OnOverrideAction,
            EventId.PrepareHit => OnPrepareHit,
            EventId.PseudoWeatherChange => OnPseudoWeatherChange,
            EventId.RedirectTarget => OnRedirectTarget,
            EventId.Residual => OnResidual,
            EventId.SetAbility => OnSetAbility,
            EventId.SetStatus => OnSetStatus,
            EventId.SetWeather => OnSetWeather,
            EventId.SideConditionStart => OnSideConditionStart,
            EventId.StallMove => OnStallMove,
            EventId.SwitchIn => OnSwitchIn,
            EventId.SwitchOut => OnSwitchOut,
            EventId.Swap => OnSwap,
            EventId.TakeItem => OnTakeItem,
            EventId.WeatherChange => OnWeatherChange,
            EventId.TerrainChange => OnTerrainChange,
            EventId.TrapPokemon => OnTrapPokemon,
            EventId.TryAddVolatile => OnTryAddVolatile,
            EventId.TryEatItem => OnTryEatItem,
            EventId.TryHeal => OnTryHeal,
            EventId.TryHit => OnTryHit,
            EventId.TryHitField => OnTryHitField,
            EventId.TryHitSide => OnTryHitSide,
            EventId.Invulnerability => OnInvulnerability,
            EventId.TryMove => OnTryMove,
            EventId.TryPrimaryHit => OnTryPrimaryHit,
            EventId.Type => OnType,
            EventId.UseItem => OnUseItem,
            EventId.Update => OnUpdate,
            EventId.Weather => OnWeather,
            EventId.WeatherModifyDamage => OnWeatherModifyDamage,
            EventId.ModifyDamagePhase1 => OnModifyDamagePhase1,
            EventId.ModifyDamagePhase2 => OnModifyDamagePhase2,
            EventId.SideStart => OnSideStart,
            EventId.SideRestart => OnSideRestart,
            EventId.SideResidual => OnSideResidual,
            EventId.SideEnd => OnSideEnd,
            EventId.FieldStart => OnFieldStart,
            EventId.FieldRestart => OnFieldRestart,
            EventId.FieldResidual => OnFieldResidual,
            EventId.FieldEnd => OnFieldEnd,
            _ => null,
        };
    }
}