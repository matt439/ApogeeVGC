...
Console.WriteLine($"[SyncSimulator.OnChoiceRequested] Choice requested for {e.SideId}");
  ...
  Side side = Battle!.Sides.First(s => s.Id == e.SideId);
   ...
   Console.WriteLine($"[SyncSimulator.OnChoiceRequested] Received choice for {e.SideId}: {choice.Actions.Count} actions");

     // If the choice is empty, use AutoChoose
   if (choice.Actions.Count == 0)
   {
        Console.WriteLine($"[SyncSimulator.OnChoiceRequested] Empty choice, using AutoChoose for {e.SideId}");
side.AutoChoose();
            choice = side.GetChoice();
         Console.WriteLine($"[SyncSimulator.OnChoiceRequested] After AutoChoose: {choice.Actions.Count} actions");
        }
...
player.UpdateMessages(e.Messages);
    ...
    {
        Console.WriteLine($"[SyncSimulator.OnBattleEnded] Battle ended, winner: {e.Winner ?? "tie"}");
    }
    ...
    private IPlayer CreatePlayer(SideId sideId, PlayerOptions options)
{
  ...
  Player.PlayerType.Random => new PlayerRandom(sideId, options, this),
            Player.PlayerType.Console => throw new NotSupportedException("Console player cannot be used in synchronous mode"),
       Player.PlayerType.Gui => throw new NotSupportedException("GUI player cannot be used in synchronous mode"),
