...
public partial class Driver
{
    ...
    /// <summary>
    /// Runs a battle with Greedy (Player 1) vs Random (Player 2) directly on the calling thread.
    /// Same pattern as RunBattleWithPrebuiltTeamsDirect but Player 1 is Greedy.
    /// </summary>
    private (SimulatorResult Result, int Turn) RunGreedyBattleDirect(
        List<PokemonSet> team1,
        List<PokemonSet> team2,
        int team1Seed,
        int team2Seed,
    ...
    /// <summary>
    /// Runs a battle with standalone MCTS (Player 1) vs Random (Player 2) directly on the calling thread.
    /// No DL model dependencies — uses heuristic evaluation and uniform priors.
    /// </summary>
    private (SimulatorResult Result, int Turn) RunMctsStandaloneBattleDirect(
        List<PokemonSet> team1,
        List<PokemonSet> team2,
        int team1Seed,
        int team2Seed,
        ...
        Mcts.MctsConfig? mctsConfig = null)
    {
        ...
        try
        {
            PlayerOptions player1Options = new()
            {
                Type = Player.PlayerType.MctsStandalone,
                Name = "MCTS-Standalone",
                Team = team1,
                Seed = new PrngSeed(player1Seed),
                PrintDebug = debug,
    ...
    /// <summary>
    /// Runs a battle with MCTS-DL (Player 1) vs Random (Player 2) directly on the calling thread.
    /// Uses DL model policy priors and value evaluation, no information tracking.
    /// </summary>
    private (SimulatorResult Result, int Turn) RunMctsDLBattleDirect(
        List<PokemonSet> team1,
        List<PokemonSet> team2,
        int team1Seed,
        int team2Seed,
    ...
    /// <summary>
    /// Runs a battle with DL-Greedy (Player 1) vs Random (Player 2) directly on the calling thread.
    /// Pure model inference, no search — picks the highest-probability policy action each turn.
    /// </summary>
    private (SimulatorResult Result, int Turn) RunDLGreedyBattleDirect(
        List<PokemonSet> team1,
        List<PokemonSet> team2,
        int team1Seed,
        int team2Seed,
        ...
        bool debug)
    {
        ...
        try
        {
            PlayerOptions player1Options = new()
            {
                Type = Player.PlayerType.DLGreedy,
                Name = "DL-Greedy",
                Team = team1,
                Seed = new PrngSeed(player1Seed),
                PrintDebug = debug,
