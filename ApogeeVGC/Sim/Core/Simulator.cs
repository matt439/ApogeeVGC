   ...
   if (Battle != null)
 {
  ...
  _cancellationTokenSource?.Dispose();
   ...
   Battle!.Start();
      ...
      Console.WriteLine("[Simulator.RunBattleLoop] Battle.Start() returned - battle should be complete");
        }
  catch (Exception ex)
        {
...
IPlayer player = GetPlayer(e.SideId);
 ...
 e.Request,
  ...
  {
      Console.WriteLine($"[Simulator.OnChoiceRequested] Timeout for {e.SideId}, auto-choosing");
    ...
    SideId = e.SideId,
         Choice = side.GetChoice(),
          Success = true
            ...
            {
   Console.WriteLine($"[Simulator.OnChoiceRequested] Error for {e.SideId}: {ex.Message}");
...
IPlayer player = GetPlayer(e.SideId);
  ...
  private void OnClearMessagesRequested(object? sender, EventArgs e)
  {
    ...
    private async Task ProcessChoiceResponsesAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                Console.WriteLine($"[Simulator.ProcessChoiceResponsesAsync] Processing choice for {response.SideId}");
        ...
        catch (OperationCanceledException)
 {
    ...
    {
        Console.WriteLine("[Simulator.OnAllChoicesReceived] All choices received, committing and releasing battle loop");
...
internal record ChoiceResponse
{
    ...
    required public Choice Choice { get; init; }
