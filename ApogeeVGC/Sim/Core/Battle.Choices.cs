...
public partial class Battle
{
    public void SubmitChoice(PlayerId playerId, Choice choice)
    {
        lock (ChoiceLock)
        {
            ...
            else if (IsReadyForForceSwitchProcessing())
            {
                ...
                if (Player1PendingChoice != null && ChoiceTools.IsSwitchChoice(Player1PendingChoice.Value))
                {
                    PerformSwitch(PlayerId.Player1, Player1PendingChoice.Value);
                ...
                else if (Player2PendingChoice != null && ChoiceTools.IsSwitchChoice(Player2PendingChoice.Value))
                {
                    PerformSwitch(PlayerId.Player2, Player2PendingChoice.Value);
                    CLearPendingChoice(PlayerId.Player2);
    ...
    private bool CanPlayerSubmitChoice(PlayerId playerId)
    {
        ...
        lock (ChoiceLock)
        {
            return playerId switch
            {
                PlayerId.Player1 => BattleTools.CanSubmitChoice(Player1State),
                PlayerId.Player2 => BattleTools.CanSubmitChoice(Player2State),
                _ => false,
            };
        }
    }
