using ApogeeVGC.Data;
using ApogeeVGC.Sim.GameObjects;
using ApogeeVGC.Sim.Moves;
using ApogeeVGC.Sim.PokemonClasses;
using ApogeeVGC.Sim.Stats;
using ApogeeVGC.Sim.Utils;

namespace ApogeeVGC.Sim.Generators;

public struct MoveSetup
{
    public MoveId Id { get; init; }
    public int PpUp
    {
        get;
        init
        {
            if (value is < 0 or > 3)
            {
                throw new ArgumentOutOfRangeException(nameof(value), "PP Up must be between 0 and 3.");
            }
            field = value;
        }
    } = 0;
    public MoveSetup(MoveId id, int ppUp = 0)
    {
        Id = id;
        PpUp = ppUp;
    }
}

public static class PokemonBuilder
{
    public static Pokemon Build(
        Library library,
        SpecieId specie,
        MoveSetup[] moves,
        AbilityId ability,
        StatsTable evs,
        NatureType nature,
        MoveType terraType,
        ItemId? item = null,
        bool pringDebug = false,
        StatsTable? ivs = null,
        string? nickname = null,
        bool shiny = false,
        int level = 50)
    {
        List<Move> movesList = [];

        foreach (MoveSetup moveSetup in moves)
        {
            if (!library.Moves.TryGetValue(moveSetup.Id, out Move? move))
            {
                throw new ArgumentException($"Move {moveSetup.Id} not found in library.");
            }
            move.PpUp = moveSetup.PpUp; // Set PP Up
            movesList.Add(move);
        }

        Specie spec = library.Species[specie] ??
                      throw new ArgumentException($"Specie {specie} not found in library.");
        Nature nat = library.Natures[nature] ??
                     throw new ArgumentException($"Nature {nature} not found in library.");

        Pokemon pokemon = new(spec, evs, ivs ?? StatsTable.PerfectIvs, nat, level)
        {
            Moves = movesList.ToArray(),
            Item = item is not null ?
                library.Items[item.Value] ?? throw new ArgumentException($"Item {item} not found in library.")
                : null,
            Ability = library.Abilities[ability] ??
                      throw new ArgumentException($"Ability {ability} not found in library."),
            Evs = evs,
            Name = nickname ?? library.Species[specie].Name,
            Shiny = shiny,
            PrintDebug = pringDebug,
            TeraType = terraType,
        };

        return PokemonValidator.IsValid(library, pokemon) ? pokemon
            : throw new ArgumentException("Invalid Pokemon configuration.");
    }

    public static PokemonSet BuildTestSet(Library library, bool printDebug = false)
    {
        return new PokemonSet()
        {
            Pokemons =
            [
                Build(
                    library,
                    SpecieId.CalyrexIce,
                    [new MoveSetup(MoveId.IceBasic, 3),
                        new MoveSetup(MoveId.PsychicBasic, 3),
                        new MoveSetup(MoveId.GhostBasic, 3),
                        new MoveSetup(MoveId.WaterBasic, 3)],
                    AbilityId.NullAbility,
                    new StatsTable { Hp = 236, Atk = 36, SpD = 236 },
                    NatureType.Adamant,
                    MoveType.Water,
                    null,
                    printDebug
                ),
                Build(
                    library,
                    SpecieId.Miraidon,
                    [new MoveSetup(MoveId.DragonBasic, 3),
                        new MoveSetup(MoveId.ElectricBasic, 3),
                        new MoveSetup(MoveId.FairyBasic, 3),
                        new MoveSetup(MoveId.RockBasic, 3)],
                    AbilityId.NullAbility,
                    new StatsTable { Hp = 236, Def = 52, SpA = 124, SpD = 68, Spe = 28 },
                    NatureType.Modest,
                    MoveType.Fairy,
                    null,
                    printDebug
                ),
                Build(
                    library,
                    SpecieId.Ursaluna,
                    [new MoveSetup(MoveId.NormalBasic, 3),
                        new MoveSetup(MoveId.GroundBasic, 3),
                        new MoveSetup(MoveId.RockBasic, 3),
                        new MoveSetup(MoveId.GhostBasic, 3)],
                    AbilityId.NullAbility,
                    new StatsTable { Hp = 108, Atk = 156, Def = 4, SpD = 116, Spe = 124 },
                    NatureType.Adamant,
                    MoveType.Ghost,
                    null,
                    printDebug
                ),
                Build(
                    library,
                    SpecieId.Volcarona,
                    [new MoveSetup(MoveId.FireBasic, 3),
                        new MoveSetup(MoveId.BugBasic, 3),
                        new MoveSetup(MoveId.WaterBasic, 3),
                        new MoveSetup(MoveId.PoisonBasic, 3)],
                    AbilityId.NullAbility,
                    new StatsTable { Hp = 252, Def = 196, SpD = 60 },
                    NatureType.Bold,
                    MoveType.Water,
                    null,
                    printDebug
                ),
                Build(
                    library,
                    SpecieId.Grimmsnarl,
                    [new MoveSetup(MoveId.FairyBasic, 3),
                        new MoveSetup(MoveId.DarkBasic, 3),
                        new MoveSetup(MoveId.GhostBasic, 3),
                        new MoveSetup(MoveId.SteelBasic, 3)],
                    AbilityId.NullAbility,
                    new StatsTable { Hp = 236, Atk = 4, Def = 140, SpD = 116, Spe = 12 },
                    NatureType.Careful,
                    MoveType.Ghost,
                    null,
                    printDebug
                ),
                Build(
                    library,
                    SpecieId.IronHands,
                    [new MoveSetup(MoveId.ElectricBasic, 3),
                        new MoveSetup(MoveId.FightingBasic, 3),
                        new MoveSetup(MoveId.PsychicBasic, 3),
                        new MoveSetup(MoveId.BugBasic, 3)],
                    AbilityId.NullAbility,
                    new StatsTable { Atk = 236, SpD = 236, Spe = 36 },
                    NatureType.Adamant,
                    MoveType.Bug,
                    null,
                    printDebug
                ),
            ],
        };
    }
}