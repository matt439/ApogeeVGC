...
public static class TeamGenerator
{
 ...
 Ability = AbilityId.FlameBody,
  ...
  Nature = library.Natures[NatureId.Adamant],
    ...
    public static List<PokemonSet> ParseShowdownTeam(string packedTeam, Library library)
  {
  Console.WriteLine($"[TeamGenerator] Received team data (length: {packedTeam.Length})");
     Console.WriteLine($"[TeamGenerator] Using test team instead");
        return GenerateTestTeam(library);
    }
}
