...
ModdedDex::ModdedDex(const std::string& mod)
    : is_base(mod == "base"),
    current_mod(mod),
    data_dir(is_base ? DATA_DIR.string() : (MODS_DIR / mod).string()),
    data_cache(std::nullopt),
    text_cache(std::nullopt),
    formats(this),
    abilities(this),
    items(this),
    moves(this),
...
ModdedDex& ModdedDex::mod(const std::string& mod)
{
    if (!dexes["base"].mods_loaded) dexes["base"].include_mods();
    return dexes[mod.empty() ? "base" : mod].include_data();
}

ModdedDex& ModdedDex::for_gen(int gen)
{
    if (gen == 0) return *this;
    return mod("gen" + std::to_string(gen));
}

ModdedDex& ModdedDex::for_format(const Format& format)
{
    if (!mods_loaded) include_mods();
    std::string mod = formats.get(format).mod; // Assume .mod is a string member
    return dexes[mod.empty() ? std::string(BASE_MOD) : mod].include_data();
}
...
T& ModdedDex::mod_data(DataType data_type, const std::string& id)
{
    ...
    if (is_base) return table.at(id);

    DexTable<T>& parent_table = dexes[parent_mod].get_table<T>(data_type);
    if (table.at(id) != parent_table.at(id)) return table.at(id);
...
std::string ModdedDex::effect_to_string() const
{
    return name;
}
...
{
    auto type_data = types.get(target_type);
    if (!type_data) return 0;
    auto it = type_data->damage_taken.find(source_type);
    if (it == type_data->damage_taken.end()) return 0;
    switch (it->second) {
...
int ModdedDex::get_effectiveness(const std::string& source_type, const std::vector<std::string>& target_types) const
{
    int total = 0;
    for (const auto& t : target_types) {
...
std::optional<std::string> ModdedDex::get_alias(const std::string& id)
{
    const auto& aliases_map = load_aliases();
