    ...
    private partial Dictionary<ConditionId, Condition> CreateConditionsAbc()
    {
        return new Dictionary<ConditionId, Condition>
        {
            ...
            [ConditionId.Bide] = new()
            {
                ...
                OnLockMove = new OnLockMoveEventInfo(MoveId.Bide),
                ...
                OnBeforeMove = new OnBeforeMoveEventInfo((battle, pokemon, _, _) =>
                {
                    ...
                    {
                        battle.Add("-end", pokemon, "move: Bide");
                        Pokemon? target = effectState.LastDamageSource;

                        // Check if we have a valid target and damage
                        if (target == null || (effectState.TotalDamage ?? 0) == 0)
                        {
                            battle.AttrLastMove("[still]");
                            battle.Add("-fail", pokemon);
                            return BoolVoidUnion.FromBool(false);
                        ...
                        {
                            // Get a random target
                            Pokemon? possibleTarget = battle.GetRandomTarget(pokemon,
                                _library.Moves[MoveId.Pound]);
                            if (possibleTarget == null)
                            {
                                battle.Add("-miss", pokemon);
                                return BoolVoidUnion.FromBool(false);
                            }
                        ...
                        // Create Bide attack move
                        ActiveMove bideMove = new()
                        {
                            Id = MoveId.Bide,
                            Name = "Bide",
                            Accuracy = IntTrueUnion.FromTrue(),
                            Damage = (effectState.TotalDamage ?? 0) * 2,
                            Category = MoveCategory.Physical,
                            Priority = 1,
                            Flags = new MoveFlags { Contact = true, Protect = true },
                            Type = MoveType.Normal,
                            MoveSlot = new MoveSlot(),
                        };

                        battle.Actions.TryMoveHit(target, pokemon, bideMove);
                        pokemon.RemoveVolatile(_library.Conditions[ConditionId.Bide]);
                        return BoolVoidUnion.FromBool(false);
                    }

                    // On other turns, just show activation message
                    battle.Add("-activate", pokemon, "move: Bide");
                    return BoolVoidUnion.FromVoid();
                }),
                ...
                OnEnd = new OnEndEventInfo((battle, pokemon) =>
                {
                    battle.Add("-end", pokemon, "move: Bide", "[silent]");
                }),
            },
