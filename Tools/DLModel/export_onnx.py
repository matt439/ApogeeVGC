...
def export_opponent(checkpoint_path: str, output_path: str) -> None:
    from format_spec import FormatSpec, VGC
    ...
    model = OpponentPredictionNet(
        ...
        pokemon_dim=train_args['pokemon_dim'],
        hidden_dim=train_args['hidden_dim'],
        num_trunk_layers=train_args.get('num_trunk_layers', 2),
        trunk_dropout=train_args.get('trunk_dropout', 0.3),
        head_dim=train_args.get('head_dim', 64),
    )
    ...
    policy_names = [f'opp_policy_{chr(97+i)}' for i in range(fmt.num_leads)]
    ...
    print(f'Exported OpponentPredictionNet to {output_path}')
    print(f'  Format: {fmt.name} ({num_slots} slots, '
          f'{fmt.numeric_dim}D numeric)')
    print(f'  Outputs: {policy_names}')
    print(f'Exported vocab to {vocab_out}')
    print(f'  Species: {vocab["num_species"]}, Actions: {vocab["num_actions"]}')
    print(f'  Epoch: {checkpoint.get("epoch", "?")}')
    print(f'  Val loss: {checkpoint.get("val_loss", "?"):.4f}')
...
def main():
    ...
    parser.add_argument('model_type',
                        choices=['battle', 'team_preview', 'opponent'])
    ...
    elif args.model_type == 'opponent':
        cp = args.checkpoint or 'opponent_model.pt'
        out = args.output or 'opponent_model.onnx'
        export_opponent(cp, out)
    else:
